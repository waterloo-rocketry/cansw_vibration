...
_Bool build_debug_printf(const uint8_t *data,
                        can_msg_t *output);
...
_Bool build_actuator_cmd_analog(uint32_t timestamp,
          enum ACTUATOR_ID actuator_id,
...
_Bool build_imu_data_msg(uint16_t message_type,
                        uint32_t timestamp,
                        const uint16_t *imu_data,
                        can_msg_t *output);




_Bool build_state_est_data_msg(uint32_t timestamp,
         const float *data,
...
_Bool build_analog_data_msg(uint32_t timestamp,
                           ...
                           can_msg_t *output);
# 161 "./canlib/can_common.h"
_Bool build_temp_data_msg(uint32_t timestamp,
                         uint8_t sensor_num,
                         int32_t temp,
                         can_msg_t *output);
...
_Bool build_state_est_calibration_msg(uint32_t timestamp,
          uint8_t ack_flag,
          uint16_t apogee,
          can_msg_t *output);
...
_Bool get_state_est_data(const can_msg_t *msg,
      float *data,
      enum STATE_ID *data_id);
...
_Bool get_temp_data(const can_msg_t *msg,
                   uint8_t *sensor_num,
                   int32_t *temp);
...
_Bool get_state_est_calibration_msg(const can_msg_t *msg,
           uint8_t *ack_flag,
           uint16_t *apogee);
...
# 1 "./canlib/pic18f26k83/pic18f26k83_timer.h" 1
...
# 19 "./canlib/canlib.h" 2
# 4 "main.c" 2

# 1 "./common.h" 1
# 36 "./common.h"
static __attribute__((inline)) void BLUE_LED_ON(void) {
    LATA2 = 0;
}

static __attribute__((inline)) void BLUE_LED_OFF(void) {
    LATA2 = 1;
}

static __attribute__((inline)) void BLUE_LED_TOGGLE(void) {
    LATA2 ^= 1;
}

static __attribute__((inline)) void GREEN_LED_ON(void) {
    LATA3 = 0;
}

static __attribute__((inline)) void GREEN_LED_OFF(void) {
    LATA3 = 1;
}

static __attribute__((inline)) void GREEN_LED_TOGGLE(void) {
    LATA3 ^= 1;
}

static __attribute__((inline)) void RED_LED_ON(void) {
    LATA4 = 0;
}

static __attribute__((inline)) void RED_LED_OFF(void) {
    LATA4 = 1;
}

static __attribute__((inline)) void RED_LED_TOGGLE(void) {
    LATA4 ^= 1;
}
# 5 "main.c" 2
...
# 39 "./mcc_generated_files/drivers/spi_master.h"
...
static __attribute__((inline)) void SET_ACCEL_I2C_ADDR(uint8_t addr) {
...
uint8_t tx_pool[150];
volatile _Bool seen_can_message = 0;
...
void main(void) {
    ...
    if (f_mount(&FatFs, "", 1) == FR_OK) {
        if (f_open(&Fil, "Data.txt", 0x04 | 0x01 | 0x02) == FR_OK) {
            f_write(&Fil, "BEGIN LOG\r\n", 10, &bw);
            f_close(&Fil);
        }
    } else {
        while (f_mount(&FatFs, "", 1) != FR_OK) {
            if (millis() - last_status_millis > 500) {
    ...
    while (1) {
        ...
        if (millis() - last_accel_millis > 500) {
            fxls_init();

            LATB3 = 1;
            while (!data_ready()) {
                uint8_t sys_mode;
                i2c_read_reg8(0x31, 0x14, &sys_mode);




                uint8_t buf_status;
                i2c_read_reg8(0x31, 0x0b, &buf_status);




                uint8_t status;
                i2c_read_reg8(0x31, 0x00, &status);
# 151 "main.c"
                uint8_t who;
                i2c_read_reg8(0x31, 0x13, &who);

                uint8_t rev;
                i2c_read_reg8(0x31, 0x12, &rev);
                }
            LATB3 = 0;
...
static void can_msg_handler(const can_msg_t *msg) {
    ...
    if (get_board_unique_id(msg) == 0x0C) {
        return;
    }
...
static void send_status_ok(void) {
    ...
    build_board_stat_msg(millis(), E_NOMINAL, ((void*)0), 0, &board_stat_msg);
    ...
    if (f_open(&Fil, "Data.txt", 0x30 | 0x01 | 0x02) == FR_OK) {
...
static void send_status_err(void) {
    can_msg_t board_stat_msg;
    build_board_stat_msg(millis(), E_MISSING_CRITICAL_BOARD, ((void*)0), 0, &board_stat_msg);
    txb_enqueue(&board_stat_msg);

    RED_LED_TOGGLE();
}

static void __attribute__((picinterrupt(("")))) interrupt_handler(void) {
    if (PIR5) {
