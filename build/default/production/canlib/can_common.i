...
# 4 "canlib/can_common.c" 2
# 14 "canlib/can_common.c"
...
_Bool build_debug_printf(const uint8_t *input_data,
                        can_msg_t *output)
{
    if (!output) { return 0; }
    if (!input_data) { return 0; }

    output->sid = 0x1E0 | 0x0C;
    for (int i = 0; i < 8; ++i) {
...
_Bool get_state_est_data(const can_msg_t *msg, float *data, enum STATE_ID *data_id){
 if(!msg) { return 0; }
 if(!data) { return 0; }
 if(!data_id) { return 0; }
...
_Bool get_temp_data(const can_msg_t *msg,
                   ...
                   int32_t *temp)
{
    if (!msg || !sensor_num || !temp) { return 0; }
    if (get_message_type(msg) != 0x540) { return 0; }
           ...
           uint16_t *apogee)
{
 ...
 if (!apogee) { return 0; }
 if (get_message_type(msg) != 0x620) { return 0; }
